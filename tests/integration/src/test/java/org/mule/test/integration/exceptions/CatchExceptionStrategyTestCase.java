...
public class CatchExceptionStrategyTestCase extends FunctionalTestCase
{
    ...
    {
        assertResponse(runFlow("continueProcessingActualMessage", getTestMuleMessage(JSON_REQUEST)).getMessage());
    }
	...
	{
        MuleMessage result = runFlow("catchWithComponent", getTestMuleMessage(MESSAGE)).getMessage();
    ...
    {
        MuleMessage result = runFlow("fullyDefinedCatchExceptionStrategyWithComponent", getTestMuleMessage(MESSAGE)).getMessage();
        assertThat(result,IsNull.<Object>notNullValue());
